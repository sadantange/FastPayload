...
class SAM(nn.Module):
    ...
    def __init__(self, class_num, max_byte_len=50, kernel_size = [3, 4], \
        d_dim=256, dropout=0.1, filters=256, batch_size=64):
        ...
        self.position_code = np.random.randint(0, max_byte_len, (self.batch_size, max_byte_len))
        self.position_code = torch.from_numpy(self.position_code).long().cuda()

    def forward(self, x):
        x = x.long()
        out = self.byteembedding(x) + self.posembedding(self.position_code)
...
if __name__ == '__main__':
    input_shape = (64, 50)
    sam = SAM(num_class=5, batch_size=64)
    summary(sam, input_shape)    